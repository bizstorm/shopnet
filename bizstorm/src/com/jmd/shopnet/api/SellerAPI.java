...
public class SellerAPI {
	...
	Logger log = Logger.getLogger(SellerAPI.class.getName());
	...
	public List<Business> searchRetailers(
		...
		try {
			...
			log.logp(Level.SEVERE, SellerAPI.class.getName(), "searchRetailers", "Shop search failed. " + e.getMessage(), e);
		...
		else
		{
			...
			if(custList.size() > 0)
			{
				...
				try {
					...
					log.logp(Level.SEVERE, SellerAPI.class.getName(), "insertCustomer", "insert customer failed. " + e.getMessage(), e);
				}
			}
		}
	}
		...
		else
		{
			...
			if(custList.size() > 0)
			{
				...
				try {
					...
					log.logp(Level.SEVERE, SellerAPI.class.getName(), "genPictureName", "generate Picture Name failed. " + e.getMessage(), e);
				}
			}
		}
		return map;
